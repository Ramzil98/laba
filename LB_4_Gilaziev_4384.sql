...
VALUES
    ('Артемов Игорь Семенович', '1998-06-19', '2019-03-07', 'Менеджер', 'senior', 50000, 1, true),
    ('Афанасьев Сергей Алексадрович', '1982-12-10', '2016-07-25', 'Разработчик', 'middle', 40000, 2, true),
    ('Закиуллин Рамиль Рустемович', '1997-12-10', '2019-06-10', 'Тестировщик', 'jun', 30000, 1, false),
    ('Козлова Марина Сергеевна', '1989-08-22', '2022-01-28', 'Аналитик', 'senior', 60000, 2, true),
    ('Морозов Артем Владимирович', '1999-08-20', '2021-04-15', 'Дизайнер', 'middle', 45000, 1, true),
    ('Гилазиев Рамзиль Фаритович', '1998-11-06', '2020-09-05', 'Руководитель проектов', 'lead', 80000, 3, true);
...
VALUES
    ('Отдел разработки', 'Афанасьев Сергей Алексадрович', 10),
    ('Отдел тестирования', 'Закиуллин Рамиль Рустемович', 5),
    ('Отдел управления проектами', 'Козлова Марина Сергеевна', 7),
...
сотрудников по уровням, общий размер оплаты труда до и после индексации, общий размер премии, общая сумма зарплат(+премии) до индексации, общая сумма зарплат(+премии) после индексации, разница в %
SELECT
    d.department_name,
    MAX(e.full_name) AS department_head,
    COUNT(e.employee_id) AS employee_count,
    COUNT(CASE WHEN e.employee_level = 'jun' THEN 1 END) AS junior_count,
    COUNT(CASE WHEN e.employee_level = 'middle' THEN 1 END) AS middle_count,
    COUNT(CASE WHEN e.employee_level = 'senior' THEN 1 END) AS senior_count,
    COUNT(CASE WHEN e.employee_level = 'lead' THEN 1 END) AS lead_count,
    ...
    SUM(e.indexed_salary + e.salary * e.bonus_coefficient) AS total_payment_after_index_with_bonus,
    (SUM(e.indexed_salary + e.salary * e.bonus_coefficient) / SUM(e.salary + e.salary * e.bonus_coefficient)) * 100 AS percentage_difference,
    COUNT(CASE WHEN er.rating = 'A' THEN 1 END) AS a_count,
    COUNT(CASE WHEN er.rating = 'B' THEN 1 END) AS b_count,
    COUNT(CASE WHEN er.rating = 'C' THEN 1 END) AS c_count,
    COUNT(CASE WHEN er.rating = 'D' THEN 1 END) AS d_count,
    COUNT(CASE WHEN er.rating = 'E' THEN 1 END) AS e_count,
...
SELECT
    ...
    COUNT(CASE WHEN er.rating = 'D' THEN 1 END) AS d_count,
    COUNT(CASE WHEN er.rating = 'E' THEN 1 END) AS e_count,
    AVG(EXTRACT(YEAR FROM AGE(CURRENT_DATE, e.start_date))) AS average_experience,
